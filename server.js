...
const serviceVerification = require('./utils/serviceVerification');
...
const register = registryFunction(async ({ name, ipv4, port, secretHash }) => {
  const isCorrectHash = await serviceVerification.isCorrectHash(secretHash);
  let message = 'The validation of the secret key is not passed';
...
const unregister = registryFunction(async ({ name, secretHash }) => {
  const isCorrectHash = await serviceVerification.isCorrectHash(secretHash);
  let message = 'The validation of the secret key is not passed';
